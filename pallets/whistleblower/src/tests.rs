...
/// Domain tag matching the pallet constant.
const COMMITMENT_DOMAIN: &[u8] = b"BelizeChainWhistleblowerV1";

/// Compute the commitment for a given account and secret.
fn compute_commitment(account: u64, secret: [u8; 32]) -> [u8; 32] {
    let mut preimage = Vec::from(COMMITMENT_DOMAIN);
    preimage.extend_from_slice(&codec::Encode::encode(&account));
...
fn submit_report_works() {
    new_test_ext().execute_with(|| {
        let nonce = [1u8; 32];
        let alias = compute_commitment(ALICE, nonce);
        ...
        assert_eq!(report.commitment, alias);
...
fn submit_report_all_categories_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Whistleblower::submit_report(
            RuntimeOrigin::signed(ALICE),
            compute_commitment(ALICE, nonce),
            TARGET,
            sample_evidence_hash(),
            0,
        ));
        assert_eq!(
            Whistleblower::reports(1).unwrap().category,
        ...
        assert_ok!(Whistleblower::submit_report(
            ...
            compute_commitment(BOB, nonce),
            TARGET,
            sample_evidence_hash(),
            1,
        ));
        assert_eq!(
            Whistleblower::reports(2).unwrap().category,
            ReportCategory::SystematicAbuse
        );

        // ChainExploit (2)
        assert_ok!(Whistleblower::submit_report(
            RuntimeOrigin::signed(CHARLIE),
            compute_commitment(CHARLIE, nonce),
            TARGET,
            sample_evidence_hash(),
            2,
        ));
        assert_eq!(
            Whistleblower::reports(3).unwrap().category,
            ReportCategory::ChainExploit
        );
    });
}
...
fn claim_reward_wrong_account_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Whistleblower::claim_reward(RuntimeOrigin::signed(BOB), report_id, nonce,),
            Error::<Test>::CommitmentMismatch
        );
    });
}
...
fn claim_reward_not_verified_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Whistleblower::claim_reward(RuntimeOrigin::signed(ALICE), 1, nonce,),
            Error::<Test>::InvalidReportStatus
        );
    });
}
...
fn claim_reward_report_not_found_fails() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Whistleblower::claim_reward(RuntimeOrigin::signed(ALICE), 999, [0u8; 32],),
            Error::<Test>::ReportNotFound
        );
    });
}
...
fn claim_reward_double_claim_fails() {
    new_test_ext().execute_with(|| {
        ...
        // Second claim — status is now Claimed (terminal state from P0-3)
        assert_noop!(
            Whistleblower::claim_reward(RuntimeOrigin::signed(ALICE), report_id, nonce,),
            Error::<Test>::InvalidReportStatus
        );
    });
}
...
fn report_category_from_u8_covers_all() {
    ...
    assert_eq!(
        ReportCategory::from_u8(1),
        Some(ReportCategory::SystematicAbuse)
    );
    assert_eq!(
        ReportCategory::from_u8(2),
        Some(ReportCategory::ChainExploit)
    );
...
fn pool_depletes_across_multiple_verified_reports() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Whistleblower::submit_report(
            RuntimeOrigin::signed(ALICE),
            compute_commitment(ALICE, nonce1),
            TARGET,
            sample_evidence_hash(),
            0,
        ));
        ...
        assert_ok!(Whistleblower::submit_report(
            RuntimeOrigin::signed(CHARLIE),
            compute_commitment(CHARLIE, nonce3),
            TARGET,
            sample_evidence_hash(),
            0,
        ));
        assert_noop!(
            Whistleblower::review_report(RuntimeOrigin::root(), 3, 0, sample_reasoning_hash(),),
            Error::<Test>::InsufficientPool
        );
    });
}
    ...
    new_test_ext().execute_with(|| {
        ...
        loop {
            ...
            let result = Whistleblower::submit_report(
                RuntimeOrigin::signed(ALICE),
                compute_commitment(ALICE, nonce_32),
                TARGET,
                sample_evidence_hash(),
                0,
            );
...
fn different_nonces_produce_different_aliases() {
    new_test_ext().execute_with(|| {
        let nonce1 = [1u8; 32];
        let nonce2 = [2u8; 32];
        let alias1 = compute_commitment(ALICE, nonce1);
        let alias2 = compute_commitment(ALICE, nonce2);
...
fn same_nonce_different_accounts_produce_different_aliases() {
    let nonce = [1u8; 32];
    let alias_alice = compute_commitment(ALICE, nonce);
    let alias_bob = compute_commitment(BOB, nonce);
...
fn full_lifecycle_submit_verify_claim() {
    new_test_ext().execute_with(|| {
        ...
        // 2. Submit report
        let nonce = [77u8; 32];
        let alias = compute_commitment(BOB, nonce);
...
fn full_lifecycle_submit_dismiss() {
    new_test_ext().execute_with(|| {
        ...
        let nonce = [55u8; 32];
        let alias = compute_commitment(CHARLIE, nonce);
...
fn relay_submission_sponsor_pattern() {
    ...
    // BOB is the sponsor/bond-depositor; ALICE holds the commitment secret.
    new_test_ext().execute_with(|| {
        assert_ok!(Whistleblower::fund_whistleblower_pool(
            RuntimeOrigin::signed(ALICE),
            100_000,
        ...
        let nonce = [99u8; 32];
        let alias = compute_commitment(ALICE, nonce); // ALICE is the real reporter
