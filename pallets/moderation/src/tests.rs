...
fn flag_content_already_ruled_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Moderation::add_moderator(
            RuntimeOrigin::root(),
            MODERATOR_1
        ));
        ...
        assert_ok!(Moderation::review_content(
            RuntimeOrigin::signed(MODERATOR_1),
            content_a(),
            0,
        ));
...
fn remove_moderator_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Moderation::remove_moderator(
            RuntimeOrigin::root(),
            MODERATOR_1
        ));
        assert!(!Moderation::moderator_set().contains(&MODERATOR_1));
...
fn review_content_cleared_works() {
    new_test_ext().execute_with(|| {
        ...
        // Ruling stored
        assert_eq!(
            Moderation::ruling(content_a()),
            Some(ModerationRuling::Cleared)
        );
    });
}
...
fn review_content_escalated_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Moderation::ruling(content_a()),
            Some(ModerationRuling::Escalated)
        );
    });
}
...
fn review_content_not_moderator_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Moderation::review_content(RuntimeOrigin::signed(DAVE), content_a(), 0,),
...
fn review_content_not_in_queue_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Moderation::review_content(RuntimeOrigin::signed(MODERATOR_1), content_a(), 0,),
...
fn review_content_invalid_ruling_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Moderation::review_content(RuntimeOrigin::signed(MODERATOR_1), content_a(), 3,),
...
fn submit_nawal_assessment_non_oracle_fails() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Moderation::submit_nawal_assessment(RuntimeOrigin::signed(ALICE), content_a(), 90,),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn moderation_ruling_from_index_covers_all() {
    assert_eq!(
        ModerationRuling::from_index(0),
        Some(ModerationRuling::Cleared)
    );
    assert_eq!(
        ModerationRuling::from_index(1),
        Some(ModerationRuling::Removed)
    );
    assert_eq!(
        ModerationRuling::from_index(2),
        Some(ModerationRuling::Escalated)
    );
...
fn two_moderators_rule_different_content() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Moderation::review_content(
            RuntimeOrigin::signed(MODERATOR_2),
            hash_b,
            1, // Removed
        ));
