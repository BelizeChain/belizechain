...
// Benchmarking module.
//
// The runtime uses `weights::SubstrateWeight<Runtime>`, which carries real
// benchmarked values generated by `scripts/bench_weights.sh pallet_belize_quantum`.
// The `impl WeightInfo for ()` above is only consumed by the mock runtime, which
// is why its numbers are rough estimates rather than measurements.
