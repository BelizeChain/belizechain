...
fn burn_dalla_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Economy::total_supply(), initial_supply - burn_amount);
...
fn multiple_burns_decrease_supply_correctly() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Economy::total_supply(), initial_supply - burn_amount_1);
...
fn mint_bbzd_fails_unauthorized_minter() {
    new_test_ext().execute_with(|| {
        ...
        let deposit_ref: BoundedVec<u8, ConstU32<64>> = b"DEPOSIT_123".to_vec().try_into().unwrap();
...
fn redeem_bbzd_fails_insufficient_balance() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Economy::redeem_bbzd(RuntimeOrigin::signed(user), redeem_amount, bank_account),
...
fn process_redemption_fails_invalid_id() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Economy::process_redemption(RuntimeOrigin::signed(central_bank), invalid_id),
...
fn process_redemption_fails_already_processed() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Economy::process_redemption(RuntimeOrigin::signed(central_bank), redemption_id),
...
fn update_reserves_requires_governance() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Economy::update_reserves(RuntimeOrigin::signed(unauthorized_user), new_reserves),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn multiple_users_can_mint_and_redeem() {
    new_test_ext().execute_with(|| {
        ...
        // Check balances
        assert_eq!(
            crate::BBZDBalances::<Test>::get(user1),
            1_000_000_000_000u128
        );
        assert_eq!(
            crate::BBZDBalances::<Test>::get(user2),
            2_000_000_000_000u128
        );
...
fn redeem_bbzd_fails_on_zero_amount() {
    new_test_ext().execute_with(|| {
        ...
        let bank_account: BoundedVec<u8, ConstU32<64>> = b"BANK_123".to_vec().try_into().unwrap();
...
fn concurrent_redemptions_maintain_consistency() {
    new_test_ext().execute_with(|| {
        ...
        let expected_supply =
            initial_supply - 50_000_000_000u128 - 100_000_000_000u128 - 150_000_000_000u128;
...
fn process_tourism_payment_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Balances::free_balance(tourist),
            tourist_before - amount + incentive
        );
        assert_eq!(Balances::free_balance(vendor), vendor_before + amount);
        assert_eq!(
            Balances::free_balance(treasury),
            treasury_before - incentive
        );
...
fn process_tourism_payment_insufficient_balance_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Economy::process_tourism_payment(RuntimeOrigin::signed(tourist), 3u64, huge_amount, 0,),
            Error::<Test>::InsufficientBalance
        );
    });
}
...
fn get_tourism_incentive_rate_returns_correct_rates() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Economy::get_tourism_incentive_rate(&TourismCategory::Accommodation),
            50_000
        );
        assert_eq!(
            Economy::get_tourism_incentive_rate(&TourismCategory::Dining),
            30_000
        );
        assert_eq!(
            Economy::get_tourism_incentive_rate(&TourismCategory::Tours),
            70_000
        );
        assert_eq!(
            Economy::get_tourism_incentive_rate(&TourismCategory::Transportation),
            20_000
        );
        assert_eq!(
            Economy::get_tourism_incentive_rate(&TourismCategory::Shopping),
            40_000
        );
        assert_eq!(
            Economy::get_tourism_incentive_rate(&TourismCategory::Cultural),
            80_000
        );
    });
}
...
fn inflation_routes_to_public_goods_treasury() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            pg_after > pg_before,
            "Public goods treasury should receive funds"
        );
        // Wellbeing gets 5% of PG allocation
        assert!(
            wb_after > wb_before,
            "Wellbeing treasury should receive funds"
        );
    });
}
...
fn process_tourism_payment_zero_amount() {
    new_test_ext().execute_with(|| {
        ...
        let result = Economy::process_tourism_payment(RuntimeOrigin::signed(2), 3, 0, 0);
