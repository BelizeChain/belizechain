...
> ### ✅ Sovereign root key — decided 2026-09-20
>
> The sovereign mainnet root is
> `5Cg3Ez7Upm8caDfjonnMKPZ14B3H5daWM75DkYj7yEt4XSKt`
> (Belize format `r1SaBq6Cszb9KEv69LAQyKERJyNhXFkMwx5Fy3mLXXyg9sj24`, pubkey
> `0x1af2d817a183f9b31aa21a7788d2b7b3226ee09a713c4989112bd13c79b53f5f`) — the
> founder key derived from the operator's offline recovery phrase. It is the key
> already wired into `node/src/chain_spec.rs` → `mainnet_genesis()` and funded on
> the testnet.
>
> The competing value `5GNzsEuoWvG6xcF4yMyfXwkjGYCfzaAfHQfXJ3dRWDqZdx7B` (pubkey
> `0xbee49c83…`) was a key auto-generated by `scripts/generate-production-keys.sh`
> on 2026-08-26 from a throwaway `key generate` phrase. It was never used on any
> chain and must not be treated as the root.
> `generated_keys/mainnet-keys-inventory.json` now records the founder key under
> `rootSudo.sr25519.ss58Address`, so the inventory and `chain_spec.rs` agree.
>
> `scripts/generate-mainnet-spec.sh` still **fails** when the inventory root and
> the code root differ, so future drift cannot silently reach a launch.
>
> Before launch, confirm the recovery phrase locally produces this key:
>
> ```bash
> ./target/release/belizechain-node key inspect --scheme sr25519 "<recovery phrase>"
> # publicKey must print 0x1af2d817a183f9b31aa21a7788d2b7b3226ee09a713c4989112bd13c79b53f5f
> ```
